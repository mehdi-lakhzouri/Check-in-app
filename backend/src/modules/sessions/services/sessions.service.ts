...
export class SessionsService implements OnModuleInit {
  ...
  constructor(
    ...
    this.sessionTtl = this.configService.get<number>('redis.cache.sessionTtl', 60000);
    this.statsTtl = this.configService.get<number>('redis.cache.statsTtl', 30000);
    this.capacityTtl = this.configService.get<number>('redis.cache.capacityTtl', 5000);
  }
  ...
  private async withSingleflight<T>(key: string, factory: () => Promise<T>): Promise<T> {
    ...
    if (pending) {
      this.logger.debug('Singleflight: coalescing request', { key, reqId: getCurrentRequestId() });
  ...
  private async invalidateSessionCache(sessionId: string): Promise<void> {
    try {
      ...
      this.logger.warn('Cache invalidation failed', { sessionId, error: error.message, reqId: getCurrentRequestId() });
  ...
  async create(createSessionDto: CreateSessionDto): Promise<SessionDocument> {
    this.logger.log('Creating session', { name: createSessionDto.name, reqId: getCurrentRequestId() });
    ...
    const status = createSessionDto.isOpen ? SessionStatus.OPEN : SessionStatus.SCHEDULED;
    ...
    this.logger.log('Session created', { sessionId: session._id, name: createSessionDto.name, reqId: getCurrentRequestId() });
    return session;
  }

  async findAll(filterDto: SessionFilterDto): Promise<PaginatedResult<SessionDocument>> {
    this.logger.debug('Finding sessions with filters', { filters: filterDto, reqId: getCurrentRequestId() });
  ...
  async findOne(id: string): Promise<SessionDocument> {
    ...
    try {
      ...
      const cached = await this.cacheManager.get<CacheWrapper<SessionDocument | null>>(cacheKey);
      ...
      if (cached && typeof cached === 'object' && 'data' in cached && 'cachedAt' in cached) {
        ...
        this.logger.debug('Cache HIT for session', { sessionId: id, reqId: getCurrentRequestId() });
        return cached.data;
      }
    } catch (error) {
      ...
      this.logger.warn('Cache read failed', { cacheKey, error: error.message, reqId: getCurrentRequestId() });
    }

    // Cache miss - fetch from database
    this.logger.debug('Cache MISS for session', { sessionId: id, reqId: getCurrentRequestId() });
    ...
    try {
      ...
      this.logger.warn('Cache write failed', { cacheKey, error: cacheError.message, reqId: getCurrentRequestId() });
  ...
  async update(id: string, updateSessionDto: UpdateSessionDto): Promise<SessionDocument> {
    this.logger.debug('Updating session', { sessionId: id, reqId: getCurrentRequestId() });
    ...
    this.logger.log('Session updated', { sessionId: id, reqId: getCurrentRequestId() });
    return session;
  }

  async remove(id: string): Promise<SessionDocument> {
    this.logger.debug('Deleting session', { sessionId: id, reqId: getCurrentRequestId() });
    ...
    if (this.isRedisAvailable()) {
      try {
        ...
        this.logger.warn('Failed to delete capacity key', { sessionId: id, error: error.message, reqId: getCurrentRequestId() });
        // Continue - key will expire via TTL
      }
    }

    this.logger.log('Session deleted', { sessionId: id, reqId: getCurrentRequestId() });
    return session;
  }

  async toggleOpen(id: string, isOpen: boolean): Promise<SessionDocument> {
    this.logger.debug('Toggling session open status', { sessionId: id, isOpen, reqId: getCurrentRequestId() });
  ...
  async getStats(): Promise<{
    ...
    try {
      ...
      if (cached) {
        this.logger.debug('Cache HIT for session stats', { reqId: getCurrentRequestId() });
        return cached;
      }
    } catch (error) {
      this.logger.warn('Cache read failed for stats', { error: error.message, reqId: getCurrentRequestId() });
    }
    ...
    return this.withSingleflight(cacheKey, async () => {
      ...
      try {
        ...
        this.logger.warn('Cache re-read failed for stats', { error: error.message, reqId: getCurrentRequestId() });
      }
      ...
      try {
        ...
        this.logger.warn('Cache write failed for stats', { error: cacheError.message, reqId: getCurrentRequestId() });
      }
  ...
  async reserveCapacitySlot(sessionId: string): Promise<CapacityReservation> {
    ...
    if (!result) {
      ...
      this.logger.warn('Session at capacity', { sessionId, capacity, reqId: getCurrentRequestId() });
      return {
        success: false,
        capacity,
        checkInsCount: session.checkInsCount,
    ...
    this.logger.debug('Session slot reserved', { sessionId, count: newCount, capacity, reqId: getCurrentRequestId() });
    ...
    if (this.isRedisAvailable()) {
      try {
        ...
        this.logger.warn('Failed to sync Redis counter', { sessionId, error: error.message, reqId: getCurrentRequestId() });
  ...
  async releaseCapacitySlot(sessionId: string): Promise<void> {
    ...
    if (this.isRedisAvailable()) {
      try {
        ...
        if (exists) {
          await this.redisClient.eval(DECREMENT_SCRIPT, { keys: [capacityKey] });
        }
      } catch (error) {
  ...
  async incrementCheckInCountWithCapacity(sessionId: string): Promise<SessionDocument | null> {
    ...
    if (!result) {
      // At capacity - MongoDB rejected the increment
      this.logger.warn(`Session ${sessionId} at capacity (${session.capacity})`);
      return null;
    }

    const newCount = result.checkInsCount;
    this.logger.debug(`Session ${sessionId} check-in count incremented to ${newCount}/${session.capacity}`);
  ...
  async syncCapacityCounters(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        this.logger.debug(`SCAN not available, falling back to KEYS: ${scanError.message}`);
      ...
      try {
        ...
        this.logger.debug(`MGET not available, falling back to sequential GET: ${mgetError.message}`);
        values = await Promise.all(keys.map(key => this.redisClient.get(key)));
      }
      ...
      for (let i = 0; i < keys.length; i += batchSize) {
        const batchPromises = keys.slice(i, i + batchSize).map(async (key, idx) => {
          try {
            ...
            if (redisCount) {
              ...
              this.logger.debug(`Synced capacity for session ${sessionId}: ${count}`);
              return { success: true };
            }
            return { success: false };
          } catch (itemError) {
            this.logger.warn(`Failed to sync capacity for key ${key}: ${itemError.message}`);
      ...
      if (syncedCount > 0 || failedCount > 0) {
        this.logger.log(`Capacity sync complete: ${syncedCount} synced, ${failedCount} failed`);
      }
    } catch (error) {
  ...
  async decrementCheckInCount(sessionId: string): Promise<void> {
    ...
    if (this.isRedisAvailable()) {
      try {
        ...
        this.logger.warn(`Failed to decrement Redis counter for session ${sessionId}: ${error.message}`);
  ...
  async getCapacityStatus(sessionId: string): Promise<{
    ...
    try {
      ...
      this.logger.warn(`Cache read failed for capacity status: ${error.message}`);
    }
    ...
    try {
      ...
      this.logger.warn(`Cache write failed for capacity status: ${cacheError.message}`);
    }
   ...
   */
  private async invalidateCapacityStatusCache(sessionId: string): Promise<void> {
    try {
      await this.cacheManager.del(`${CACHE_KEYS.CAPACITY_STATUS}${sessionId}`);
    } catch (error) {
      this.logger.warn(`Failed to invalidate capacity status cache: ${error.message}`);
    }
  }
  ...
  async reconcileCheckInCount(sessionId: string, actualCount: number): Promise<void> {
    this.logger.log(`Reconciling check-in count for session ${sessionId}: setting to ${actualCount}`);
