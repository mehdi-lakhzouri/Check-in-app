...
export class CheckIn {
  ...
  @ApiPropertyOptional({
    example: 'John Smith',
    description: 'Name of person who performed check-in',
  })
  @Prop({ trim: true })
  checkedInBy?: string;
  ...
  @ApiProperty({
    example: false,
    description:
      'Whether the check-in was late (after threshold delay from session start)',
  })
  ...
  @ApiProperty({
    example: true,
    description: 'Whether participant was registered at time of check-in',
  })
...
CheckInSchema.index(
  { participantId: 1, checkInTime: -1 },
  { name: 'participant_checkin_history_idx' },
);
