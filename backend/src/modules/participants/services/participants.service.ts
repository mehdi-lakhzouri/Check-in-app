...
export class ParticipantsService implements OnModuleInit {
  ...
  constructor(
    ...
    this.participantTtl = this.configService.get<number>('redis.cache.participantTtl', 600000);
   ...
   */
  private async invalidateParticipantCache(participant: ParticipantDocument): Promise<void> {
    try {
      await Promise.all([
        this.cacheManager.del(`${CACHE_KEYS.PARTICIPANT_BY_ID}${participant._id}`),
        this.cacheManager.del(`${CACHE_KEYS.PARTICIPANT_BY_QR}${participant.qrCode}`),
        this.cacheManager.del(`${CACHE_KEYS.PARTICIPANT_BY_EMAIL}${participant.email}`),
        this.cacheManager.del(CACHE_KEYS.PARTICIPANT_STATS),
      ]);
    } catch (error) {
      this.logger.warn(`Cache invalidation failed for participant ${participant._id}: ${error.message}`);
  ...
  async findOne(id: string): Promise<ParticipantDocument> {
    ...
    try {
      ...
      const cached = await this.cacheManager.get<CacheWrapper<ParticipantDocument | null>>(cacheKey);
  ...
  async remove(id: string): Promise<ParticipantDocument> {
    ...
    this.logger.log(`Removed participant ${id} from all ambassador referral lists`);

    // Recalculate points for all ambassadors who had this participant as referral
    const ambassadors = await this.participantRepository.findByStatus(ParticipantStatus.AMBASSADOR);
    for (const ambassador of ambassadors) {
      if (ambassador.referredParticipantIds.some(refId => refId.toString() === id)) {
  ...
  async bulkCreate(
    ...
    if (created > 0) {
      try {
        ...
        this.logger.debug(`Invalidated participant stats cache after bulk create (${created} created)`);
      } catch (cacheError) {
        this.logger.warn(`Failed to invalidate stats cache: ${cacheError.message}`);
      }
    }
  ...
  async getStats(): Promise<{
    ...
    try {
      ...
      this.logger.warn(`Cache read failed for participant stats: ${error.message}`);
    }
    ...
    return this.withSingleflight(cacheKey, async () => {
      ...
      try {
        ...
        this.logger.warn(`Cache re-read failed for participant stats: ${error.message}`);
      }
      ...
      try {
        ...
        this.logger.warn(`Cache write failed for participant stats: ${cacheError.message}`);
      }
  ...
  async getAmbassadorLeaderboard(limit = 10): Promise<ParticipantDocument[]> {
    ...
    // Try cache first with error handling
    try {
      const cached = await this.cacheManager.get<ParticipantDocument[]>(cacheKey);
      if (cached) {
        this.logger.debug('Cache HIT for ambassador leaderboard');
        return cached;
      }
    } catch (error) {
      this.logger.warn(`Cache read failed for ambassador leaderboard: ${error.message}`);
    }
    ...
    return this.withSingleflight(cacheKey, async () => {
      // Double-check cache after acquiring "lock"
      try {
        const cachedAgain = await this.cacheManager.get<ParticipantDocument[]>(cacheKey);
        if (cachedAgain) {
          return cachedAgain;
        }
      } catch (error) {
        this.logger.warn(`Cache re-read failed for ambassador leaderboard: ${error.message}`);
      }

      // Cache miss - fetch from database
      const leaderboard = await this.participantRepository.getAmbassadorLeaderboard(limit);
      
      // Cache with error handling
      try {
        await this.cacheManager.set(cacheKey, leaderboard, this.statsTtl);
      } catch (cacheError) {
        this.logger.warn(`Cache write failed for ambassador leaderboard: ${cacheError.message}`);
      }
  ...
  async calculateAmbassadorPoints(id: string): Promise<ParticipantDocument> {
    ...
    try {
      ...
      this.logger.warn(`Cache invalidation failed for participant ${id}: ${error.message}`);
    }
  ...
  async addReferredParticipant(
    ...
    try {
      await this.cacheManager.del(`${CACHE_KEYS.PARTICIPANT_BY_ID}${ambassadorId}`);
    } catch (error) {
  ...
  async syncOrganizationReferrals(ambassadorId: string): Promise<{
    ...
    if (!ambassador.organization) {
      throw new ValidationException([
        { field: 'organization', message: 'Ambassador has no organization set' },
      ]);
    }

    // Find all participants with the same organization (excluding the ambassador)
    const organizationMembers = await this.participantRepository.findOrganizationMembers(
      ambassador.organization,
      ambassadorId,
    );

    // Get current referral IDs for comparison
    const currentReferralIds = ambassador.referredParticipantIds.map(id => id.toString());
    ...
    const mergedReferralIds = [...new Set([...currentReferralIds, ...newMemberIds])];
  ...
  async syncAllAmbassadorReferrals(): Promise<{
    ...
    for (const ambassador of ambassadors) {
      if (ambassador.organization) {
        try {
          const result = await this.syncOrganizationReferrals(ambassador._id.toString());
          totalAdded += result.addedCount;
        } catch (error) {
          this.logger.warn(`Failed to sync referrals for ambassador ${ambassador._id}: ${error.message}`);
        }
      }
    }
  ...
  async searchAmbassadors(
    ...
    return this.participantRepository.searchAmbassadors(search, sortBy, sortOrder, page, limit);
  }
  ...
  async searchTravelGrants(
    ...
    return this.participantRepository.searchTravelGrants(search, status, sortBy, sortOrder, page, limit);
  }
  ...
  async getAmbassadorDetails(id: string) {
    ...
    const referredParticipants = referredIds.length > 0 
      ? await this.findByIds(referredIds)
      : [];
  ...
  async getTravelGrantDetails(id: string) {
    ...
    return {
      ...
      checkInProgress: {
        ...
        percentage: confirmedRegistrations.length > 0 
          ? Math.round((checkIns.length / confirmedRegistrations.length) * 100)
          : 0,
      },
      lastCheckIns,
      stats: {
        ...
        applicationStatus: participant.travelGrantApproved === null 
          ? 'pending' 
  ...
  async getAllAmbassadors(): Promise<ParticipantDocument[]> {
    return this.participantRepository.findByStatus(ParticipantStatus.AMBASSADOR);
  }
